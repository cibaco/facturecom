        ...
        <div className="flex overflow-hidden">
          <div className="flex gap-10 animate-marquee shrink-0">
            {logos.map((src, i) => (
              <div
                key={`a-${i}`}
                className="flex-none w-40 h-28 bg-gray-50 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-center p-3 hover:shadow-md hover:border-fce-orange transition-all duration-300"
              >
                <img
                  src={src}
                  alt={`Partenaire ${i + 1}`}
